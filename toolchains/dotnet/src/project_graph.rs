...
use crate::msbuild::{
    ...
    is_sdk_resolution_failure, longest_suffix_match, normalize_path_key,
};
use crate::tier2_env::{
...
/// real-path match first, then the longest matching workspace-relative suffix.
///
/// The fallback compares only the tail of the path, so a reference to a project
/// outside the workspace can match an in-workspace project that shares its
/// trailing path. See [`longest_suffix_match`] for why the leading part cannot
/// be compared.
fn resolve_reference<'index>(
    ...
    indexes
        .by_real_path
        .get(&key)
        .or_else(|| longest_suffix_match(&indexes.by_suffix, &key))
}
        ...
        Err(error) => {
            ...
            if is_sdk_resolution_failure(&message) {
                ...
                return Err(plugin_err!(
                    "{requirement}, so MSBuild evaluation cannot run.\n\nSet <property>version</property> under <property>dotnet</property> in <file>.moon/toolchains.yml</file> to have moon install that SDK, or install it yourself so the <symbol>dotnet</symbol> on PATH can satisfy the pin.\n\n{message}"
                ));
            }
            ...
            match inferred {
                Ok(tasks) => {
                    for (task_id, task) in tasks {
                        // moon derives one project per directory, so a directory
                        // holding several project files can only carry one task
                        // per id, the first file wins, which after the sort in
                        // `find_project_files` means the alphabetically first.
                        // Say so, rather than leaving the loser silently
                        // task-less.
                        if result.output.tasks.contains_key(&task_id) {
                            host_log!(
                                warn,
                                "Not inferring the <id>{}</id> task from <file>{}</file>: another project file in <id>{}</id> already claimed it. moon derives one project per directory, so give this project file its own directory to have its tasks inferred.",
                                task_id,
                                file.to_string(),
                                id
                            );
                        ...
                        result.output.tasks.insert(task_id, task);
                    }
                }
                Err(error) => {
                    host_log!(
...
pub fn extend_project_graph(
    ...
    // Read before the config is taken below, since moon merges its own
    // `version:` setting into the same object.
    let install_configured = sdk_install_configured(&input.toolchain_config);
    ...
    let Some(mut batch) = run_batch_evaluation(&input, &indexes, &eval_env, install_configured)?
    else {
        return Ok(Json(output));
    };
    ...
    for (id, files) in &indexes.files {
        let project_root = input
            .project_sources
            .get(id)
            .map(|source| input.context.get_project_root_from_source(source));
